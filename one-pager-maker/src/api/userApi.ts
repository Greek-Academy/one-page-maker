...
export const userApi = {
    ...
    useMutation({
      ...
      onSuccess: (user) => {
        ...
        queryClient.invalidateQueries({
          queryKey: [queryKeys.userId(user.id), queryKeys.users]
        });
      }
    }),
  useSearchUsersQuery: (id: string) =>
    useQuery({
  ...
  useFindUserByIDQuery: (id: string) =>
    useQuery({
  ...
  useFindUserByUIDQuery: (uid: string) =>
    useQuery({
  ...
  useIsDuplicatedIdQuery: (id: string) =>
    useQuery({
