...
import {
  ViewHistory,
  viewHistoryConverter
} from "../entity/viewHistoryType.ts";
import {
  collection,
  deleteDoc,
  doc,
...
export class ViewHistoryRepositoryImpl implements ViewHistoryRepository {
  private readonly colRef = (uid: string) =>
    collection(db, `users/${uid}/viewHistories`).withConverter(
      viewHistoryConverter
    );
  private readonly docRef = (uid: string, viewHistoryId: string) =>
    doc(db, `users/${uid}/viewHistories/${viewHistoryId}`).withConverter(
      viewHistoryConverter
    );
  ...
  async get({
    ...
    try {
      const snapshot = await this.clientManager
        .getClient()
        .get(this.docRef(uid, viewHistoryId));
  ...
  async update({
    ...
    try {
      await this.clientManager
        .getClient()
        .update(this.docRef(uid, viewHistory.id), {
          ...viewHistory,
          updated_at: serverTimestamp()
        });
    } catch (e) {
