...
export const RouteAuthGuard = ({
  ...
  const userQuery = userApi.useFindUserByUIDQuery(
    userState.data.user?.uid ?? ""
  );
  ...
  if (
    ...
    (shouldCreateUserData && userQuery.status === "pending")
  ) {
    return (
      <main
        className={
  ...
  if (
    ...
    (shouldCreateUserData && userQuery.status === "error")
  ) {
    return (
      <main>
  ...
  if (
    shouldCreateUserData &&
    userQuery.status === "success" &&
    userQuery.data === null
  ) {
    return (
      <Navigate to={"/set-id"} state={{ from: location }} replace={false} />
    );
  }
