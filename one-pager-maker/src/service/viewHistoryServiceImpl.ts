...
export class ViewHistoryServiceImpl implements ViewHistoryService {
  constructor(
    @inject(DI.ViewHistoryRepository)
  ...
  private getViewHistory(
    viewType: "edit" | "review",
    {
      uid,
      orderBy = "desc",
      lastFetched,
    ...
    return this.viewHistoryRepository.getMany(
      { uid },
      {
        orderBy: {
          field: "updated_at",
  ...
  private async setViewHistory(
    ...
    if (!userExists) {
      return Result.failure(
        new ViewHistoryServiceError("User not found", "user-not-found")
      );
    }

    // check if document exists
    const document =
      _document ?? (await this.documentRepository.get({ uid, documentId }));
    if (document === null) {
      return Result.failure(
        new ViewHistoryServiceError("Document not found", "document-not-found")
      );
    }

    // documentID と viewType が一致する ViewHistory を 1 件取得する
    const queryResult = await this.viewHistoryRepository.getMany(
      { uid },
      {
        where: [
