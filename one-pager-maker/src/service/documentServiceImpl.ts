...
export class DocumentServiceImpl implements DocumentService {
    ...
    async getDocument({uid, documentId}: {uid: string, documentId: string}): Promise<Result<Document | undefined, DocumentServiceError>> {
        try {
            ...
            if (e instanceof FirestoreError) {
                switch (e.code) {
                    case 'permission-denied':
                        return Result.failure(new DocumentServiceError(e.message, 'permission-denied'))
                    case 'not-found':
