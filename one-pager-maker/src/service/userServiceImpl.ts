...
export class UserServiceImpl implements UserService {
  ...
  async createUser(args: {
    ...
    try {
      ...
      if (currentUser === null || currentUser.uid !== args.uid) {
        return Result.failure(
          new UserServiceError(
            "You can't create user with unsigned-in user's data.",
            "permission-denied"
          )
        );
      }
      ...
      if (isDuplicated) {
        return Result.failure(
          new UserServiceError("ID is duplicated.", "duplicated-id")
        );
      }
  ...
  async findUserByID(id: string): Promise<Result<User, UserServiceError>> {
    try {
      ...
      if (!user) {
        return Result.failure(
          new UserServiceError("User not found.", "user-not-found")
        );
      }
      return Result.success(user);
    } catch (e) {
  ...
  async findUserByUID(uid: string): Promise<Result<User, UserServiceError>> {
    try {
      if (uid === "") {
        return Result.failure(
          new UserServiceError("UID is empty.", "empty-id")
        );
      }
      ...
      if (currentUser === null || currentUser.uid !== uid) {
        return Result.failure(
          new UserServiceError(
            "You can't get unsigned-in user's data.",
            "permission-denied"
          )
        );
      }
