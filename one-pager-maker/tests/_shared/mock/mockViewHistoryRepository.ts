...
import {
  ...
  QueryParams,
  sortByQuery
} from "../../../src/repository/shared/utils";
...
export class MockViewHistoryRepository implements ViewHistoryRepository {
  ...
  getMany(
    ...
    if (histories === undefined) {
      return Promise.reject(
        new MockViewHistoryRepositoryError(`User ${args.uid} not found`)
      );
    }
  ...
  async update(args: { uid: string; viewHistory: ViewHistory }): Promise<void> {
