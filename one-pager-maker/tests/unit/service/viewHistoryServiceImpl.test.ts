...
describe("ViewHistoryServiceImpl", function () {
  const viewHistoryService = container.resolve<ViewHistoryService>(
    DI.ViewHistoryService
  );
  const viewHistoryRepo = container.resolve<MockViewHistoryRepository>(
    DI.ViewHistoryRepository
  );
  ...
  const createViewHistory = async (
    num: number,
    viewType: "review" | "edit" = "edit"
  ) => {
    const result: ViewHistory[] = [];
    for (let i = 0; i < num; i++) {
      result.push(
        await viewHistoryRepo.create({
          uid: userUid,
          viewHistory: viewHistoryFactory.build({
            viewType,
  ...
  describe("getEditHistory()", () => {
    ...
    test("returns only limited data", async () => {
      ...
      const result = await viewHistoryService.getEditHistory({
        uid: userUid,
        limit: 5
      });
    ...
    test("returns data ordered by updated_at", async () => {
      ...
      expect(result[0].updated_at.toMillis()).toBeGreaterThan(
        result[1].updated_at.toMillis()
      );
    });
  ...
  describe("getReviewHistory()", () => {
    test("returns 1 data", async () => {
      ...
      const result = await viewHistoryService.getReviewHistory({
        uid: userUid
      });
  ...
  describe("setEditHistory()", async () => {
    test("creates a new history", async () => {
      ...
      const result = await viewHistoryService.setEditHistory({
        uid: userUid,
        documentId
      });
    ...
    test("updates the history", async () => {
      ...
      expect(result.value.updated_at.toMillis()).toBeGreaterThan(
        history.updated_at.toMillis()
      );
    });
  });
  ...
  describe("setReviewHistory()", async () => {
    test("creates a new history", async () => {
      await viewHistoryRepo.registerUser(userUid);
      const result = await viewHistoryService.setReviewHistory({
        uid: userUid,
        documentId
      });
